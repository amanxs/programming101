 ...
 * of a multi-process synchronization problem. The problem describes two processes, the producer and the consumer,
 * which share a common, fixed-size buffer used as a queue.
 ...
 In the same way, the consumer can go to sleep if it finds the buffer to be empty. The next time the producer puts data into the buffer,
...
public class ProducerConsumer {
    ...
    // This class has a list, producer(that adds items to the list) and Consumer(That removes items from the list)
    public static class PC{

        /*
        In PC class (A class that has both produce and consume methods), a linked list of jobs and a capacity of the list is added
        to check that producer does not produce if the list is full.
         */
        ...
        public void produce() throws InterruptedException {
            ...
            while (true) {
                synchronized (this) {
                    /**
                     * An inner loop is there before adding the jobs to list that checks if the job list is full,
        ...
        /**
         * In both the methods, we use notify at the end of all statements.
         ...
         *
         sleep() at the end of both methods just make the output of program run in step wise manner and not display everything all at once
         so that you can see what actually is happening in the program.
         */
